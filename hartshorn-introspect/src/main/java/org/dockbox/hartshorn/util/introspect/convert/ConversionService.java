 ...
 * methods for checking whether a conversion is possible and for actually performing the conversion,
 * and supports a variety of different types of converters. The types of converters that can be used
 * with this service are:
 * <ul>
 ...
 * controllers.
 *
 * <p>When either the {@link #canConvert} or {@link #convert} method is called with a {@code null}
 * value for the source or input parameter, the service may attempt to invoke a {@link
 * DefaultValueProvider} to obtain a default value for the input. It is important to note that not
 * all {@link ConversionService} implementations may support {@link DefaultValueProvider} instances,
...
public interface ConversionService {
    ...
    /**
     * Returns whether this service can convert an object of the given source type to the given
     * target type. This may be based on the presence of a converter, or on the ability to create a
    ...
    /**
     * Converts the given object to the given target type. This method will first check whether a
     * conversion is possible, and if so, will perform the conversion. If no conversion is possible,
