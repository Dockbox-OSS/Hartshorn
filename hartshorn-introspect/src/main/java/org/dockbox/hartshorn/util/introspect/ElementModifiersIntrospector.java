...
/**
 * An introspector to introspect the modifiers of an element. Modifiers include access modifiers,
 * such as {@code public} and {@code private}, as well as other modifiers, such as {@code static}
 * and {@code final}.
 *
 * <p>This introspector may be used for any element that has modifiers, such as a class, method, or
 * field. If the element does not support all modifiers, the introspector will return {@code false}
 * for all modifiers that are not supported. For example, a method cannot be {@code transient}, so
 * {@link #isTransient()} will always return {@code false} for a method.
 *
...
public interface ElementModifiersIntrospector {
    ...
    /**
     * Checks if the element has the provided modifier. If the element does not support the
    ...
    /**
     * Returns {@code true} if the element is {@code mandated}, {@code false} otherwise. Note that
     * this modifier is often implicitly set by the compiler, and is not often used in code.
     *
    ...
    /**
     * Returns {@code true} if the element is {@code synthetic}, {@code false} otherwise. Note that
     * this modifier is implicitly set by the compiler, and is not defined in the Java Language
     * Specification.
     *
