 ...
 * {@link #isClass() concrete type}, or {@link #isVariable() type variable}. This view can be used
 * to introspect various properties of the type parameter, such as its {@link #name() name},
 * {@link #upperBounds() bounds}, and whether it represents a {@link #isClass() class} or
 * {@link #isInterface() interface}.
 *
 * <p>Type parameters are divided into two groups: {@link #isInputParameter() input parameters} and
 * {@link #isOutputParameter() output parameters}. Input parameters are those that are
 * {@link #declaredBy() declared on-} and {@link #consumedBy() consumed by} the type itself, while
 * output parameters are those that are declared on the type, but consumed by its parents.
 *
...
public interface TypeParameterView extends AnnotatedElementView {
    ...
    /**
     * Returns {@code true} if this type parameter is an input parameter, or {@code false}
    ...
    /**
     * Returns {@code true} if this type parameter is an output parameter, or {@code false}
    ...
    /**
     * Returns the set of output type parameters that this input type parameter represents. This is
     ...
     * as a non-parameterized wildcard type. In such cases {@link #upperBounds()} should be used to
    ...
    /**
     * Returns {@code true} if this type parameter is bounded, or {@code false} otherwise. A type
     * parameter is bounded if it has an upper bound that is not {@link Object}. This is only
     * applicable to type variables and wildcards. For example, {@code T extends CharSequence} is
     * bounded, but {@code T} is not.
     *
    ...
    /**
     * Returns {@code true} if this type parameter is unbounded, or {@code false} otherwise. A type
     * parameter is unbounded if it has no upper bound. This is only applicable to type variables
     * and wildcards. Any other type will default to {@code true}. For example, {@code T} is
     * unbounded, but {@code T extends CharSequence} is not.
     *
    ...
    /**
     * Returns {@code true} if this type parameter is a concrete type that is not a wildcard, or
     * {@code false} otherwise. A type is considered concrete it {@link #resolvedType()} returns a
     * non-empty non-wildcard option.
     *
     * @return {@code true} if this type parameter is a concrete type that is not a wildcard, or
     * {@code false}
     */
    boolean isClass();

    /**
     * Returns {@code true} if this type parameter is a concrete type that is an interface, or
     * {@code false} otherwise. A type is considered concrete it {@link #resolvedType()} returns a
     * non-empty non-wildcard option.
     *
     * @return {@code true} if this type parameter is a concrete type that is an interface, or
     * {@code false}
     */
    boolean isInterface();
    ...
    /**
     * Returns {@code true} if this type parameter is a concrete type that is an annotation, or
     * {@code false} otherwise. A type is considered concrete it {@link #resolvedType()} returns a
     * non-empty non-wildcard option.
     *
     * @return {@code true} if this type parameter is a concrete type that is an annotation, or
     * {@code false}
     */
    ...
    /**
     * Resolves the current output type parameter to an input type parameter on its consumer. Unlike
     * {@link #definition()}, which resolves the input parameter on the declaring type, this method
