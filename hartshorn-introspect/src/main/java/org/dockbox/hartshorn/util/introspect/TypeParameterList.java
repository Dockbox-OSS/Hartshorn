...
public interface TypeParameterList extends Iterable<TypeParameterView> {
    ...
    /**
     * Returns the type parameter at the provided index. If the index is out of bounds, an empty
    ...
    /**
     * Transforms this list to a {@link List} of {@link TypeParameterView}s. The returned list is
     * immutable.
     *
    ...
    /**
     * Returns a bi-directional map of type parameters. Depending on whether the type parameter is
     * an input or output parameter, the map will contain different mappings.
     *
     * <p>If the parameter is an input parameter, the map will contain the parameter as the key, and
     * all the types it
     * represents as the values. The represented types will be resolved to their output parameters,
     * similar to {@link TypeParameterView#represents()}. If a type does not represent any outputs,
     * it will not be included in the map.
     *
     * <p>If the parameter is an output parameter, the map will contain the parameter as the key,
     * and the input
