...
public abstract class AbstractCommandContext {
    ...
    protected AbstractCommandContext(Command command, Class<?> owner) {
        this.command = command;
        for (String alias : command.value()) this.addAlias(alias);
        this.owner = Hartshorn.context().get(MetaProvider.class).lookup(owner);
        if (this.owner == null) {
            throw new IllegalArgumentException("Could not find owner information for " + owner.getSimpleName());
        }
    }
