...
import org.dockbox.hartshorn.palswap.fileparsers.ItemData;
...
public enum VariantIdentifier implements RegistryIdentifier {
    ...
    static {
        ...
        if (HartshornBootstrap.isConstructed()) {
            blockRegistryParser = Hartshorn.context().get(BlockRegistryParser.class);
        }
    }

    public static ItemData getOverriddenBlockNames() {
        if (null == overridenBlockNames) {
            if (!HartshornBootstrap.isConstructed()) return (overridenBlockNames = new ItemData());

            FileManager fm = Hartshorn.context().get(FileManager.class);
            Path file = fm.getDataFile(BlockRegistryService.class, "overridenblocknames");
    ...
    public static String getBlockNameWithoutVariant(String name) {
        ...
        return HartshornUtils.capitalize(blockName.toLowerCase(Locale.ROOT).trim());
    }
