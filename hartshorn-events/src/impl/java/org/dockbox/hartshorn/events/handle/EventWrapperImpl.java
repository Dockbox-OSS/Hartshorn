 ...
 * fired.
 */
public final class EventWrapperImpl implements Comparable<EventWrapperImpl>, EventWrapper {
    ...
    private EventWrapperImpl(Class<?> type, Class<? extends Event> eventType, Method method, int priority) {
        this.listener = null;
     ...
     */
    public static List<EventWrapperImpl> create(Class<?> type, Method method, int priority) {
        List<EventWrapperImpl> invokeWrappers = HartshornUtils.emptyConcurrentList();
        for (Class<?> param : method.getParameterTypes()) {
            if (Reflect.assigns(Event.class, param)) {
                @SuppressWarnings("unchecked")
                Class<? extends Event> eventType = (Class<? extends Event>) param;
                invokeWrappers.add(new EventWrapperImpl(type, eventType, method, priority));
            }
        }
    ...
    @Override
    public int compareTo(@NotNull EventWrapperImpl o) {
    ...
    private static boolean fastEqual(EventWrapperImpl o1, EventWrapperImpl o2) {
