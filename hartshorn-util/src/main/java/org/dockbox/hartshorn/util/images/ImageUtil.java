...
public final class ImageUtil {
    ...
    public static final int[] INDEXED_COLORS = {
            0x00000000, 0x00000000, 0x00000000, 0x00000000, 0xFF5A7E28, 0xFF6E9A30, 0xFF7FB238, 0xFF435E1E,
    ...
    public static BufferedImage scale(BufferedImage image, BufferedImage background, ScaleMode scaleMode) {
        ...
        int scaledW = imgAspectRatio > mapsAspectRatio
                ? scaledImg.getWidth()
                : (int) Math.round(scaledImg.getHeight() * imgAspectRatio);
        int scaledH = imgAspectRatio > mapsAspectRatio
        ...
        if (!mode.enabled()) {
            for (int i = 0; i < image.getHeight(); i++) {
                for (int j = 0; j < image.getWidth(); j++) {
                    ...
                    int colorIndex = closestMapColorIndex(
    ...
    private static int closestMapColorIndex(int R, int G, int B) {
        ...
        for (int i = 4; i < INDEXED_COLORS.length; i++) { // 0-3 are reserved for transparent
            double curDist = colorDistance(R, G, B,
