 ...
 * {@link ModuleActivator} annotations that are found in the application. This context is used to
 * determine which activators are available, and to retrieve the actual annotation instance.
 *
...
public class ModuleActivatorContext extends DefaultFallbackCompatibleContext implements Reportable {
    ...
    private final Map<Class<? extends Annotation>, Annotation> activators =
        ...
        for (Annotation moduleActivator : moduleActivators) {
            if (!moduleActivator.annotationType().isAnnotationPresent(ModuleActivator.class)) {
                throw new IllegalArgumentException("Annotation "
                    + moduleActivator
                    + " is not a valid module activator");
            }
     ...
     * includes all activators that are provided to this context, and will not filter out any
     * hierarchical activators.
     *
    ...
    /**
     * Returns whether this context contains an activator for the given {@link ModuleActivator}.
     * This method will only return {@code true} if the given activator is directly available in
     * this context, but not if the given activator is hierarchical through (virtual) inheritance.
     *
     * @param activator The activator to check for
     *
     * @return {@code true} if the given activator is directly available in this context,
    ...
    public boolean hasActivator(Class<? extends Annotation> activator) {
        if (!activator.isAnnotationPresent(ModuleActivator.class)) {
            throw new InvalidActivatorException("Requested activator "
                + activator.getSimpleName()
    ...
    /**
     * Returns the {@link ModuleActivator} annotation instance for the given activator type. This
     * method will only return a value if the given activator is directly available in this context,
     ...
     *
     * @return The {@link ModuleActivator} annotation instance for the given activator type, or
     * {@code null} if the given activator is not directly available in this context.
     */
