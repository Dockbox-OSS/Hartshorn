...
/**
 * A registry for external classes that can be used in HSL scripts. External classes are classes
 * that are not defined in the script itself, but are imported from outside the script runtime. This
 * registry allows for defining, removing, and querying external classes by their name or alias.
 *
 * <p>Each external class can have multiple names or aliases associated with it, allowing for
 * flexible referencing
 * within scripts. The registry ensures that class names and aliases are unique to prevent
 * conflicts. Similarly, this registry ensures that {@link ExternalClass} instances are re-used for
 * the same alias or class name.
 *
 * @since 0.7.0
 * 
 * @author Guus Lieben
...
public interface ExternalClassRegistry {
     ...
     * under its direct name. For example, a class named {@code com.example.MyClass} will be
     * accessible in scripts as {@code MyClass}.
     *
    ...
    /**
     * Defines a single external class based on the provided type view and exposes it under a custom
     * alias.
     *
     * @param typeView the type view representing the class to define.
    ...
    /**
     * Retrieves a map of all imported classes, where the keys are the names or aliases and the
     * values are the corresponding {@link ExternalClass} instances.
     *
