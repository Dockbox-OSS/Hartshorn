...
/**
 * Represents a validator for a step in the token stream. This is used to validate the token stream
 * while parsing, typically when performed through a {@link StatementParser}.
 *
 * @since 0.4.13
 *
 * @author Guus Lieben
...
public interface TokenStepValidator {
    ...
    /**
     * Attempts to consume a token of the given type. If the token is not of the given type, a
    ...
    /**
     * Attempts to consume a token of the given type. If the token is not of the given type, a
     * {@link ScriptEvaluationError} is thrown. The given {@code what} is used to provide context
     * about what kind of token was expected.
     *
     * @param type the type of token to consume
    ...
    /**
     * Attempts to consume a token of the given type, which is expected to be before another token.
     * If the token is not of the given type, a {@link ScriptEvaluationError} is thrown.
     *
    ...
    /**
     * Attempts to consume a token of the given type, which is expected to be after another token.
     * If the token is not of the given type, a {@link ScriptEvaluationError} is thrown.
     *
    ...
    /**
     * Attempts to consume a token of the given type, which is expected to be around another token.
     * If the token is not of the given type, a {@link ScriptEvaluationError} is thrown.
     *
