...
public class SpongeCommandBus extends DefaultCommandBus {
    ...
    private static CommandExecutor buildExecutor(
            AbstractRegistrationContext registrationContext, String command) {
        return (src, args) -> {
      /*
      ...
      */
            CommandSource sender =
                    SpongeConversionUtil.fromSponge(src)
                            .orElseThrow(
                                    () ->
                                            new IllegalArgumentException(
                                                    "Command sender is not a convertable source type, did a plugin call me?"));
    ...
    private static SimpleCommandContext createCommandContext(
            CommandContext ctx, @NotNull CommandSource sender, @Nullable String command) {
    /*
    ...
    */
    ...
    protected AbstractArgumentElement<?> wrapElements(List<AbstractArgumentElement<?>> elements) {
        return new SpongeArgumentElement(
                elements.stream()
    ...
    */
        DefaultCommandBus.getRegistrations()
                .forEach(
                        (alias, abstractCommand) -> {
                            ...
                            if (abstractCommand instanceof MethodCommandContext) {
                                ...
                                if (!methodContext.getCommand().inherit()
                                    ...
                                    Selene.log()
                                            .error(
                                                    "Found direct method registration of inherited command! "
                                                            + methodContext.getLocation());
                                }

                            }
                            else if (abstractCommand instanceof CommandInheritanceContext) {
                                CommandInheritanceContext inheritanceContext =
                                        (CommandInheritanceContext) abstractCommand;
                                ...
                                Selene.log()
                                        .error(
                                                "Found unknown context type ["
                                                        + abstractCommand.getClass().getCanonicalName()
                                                        + "]");
                            }
                            ...
                            if (null != spec) {
                                ...
                                Selene.log()
                                        .warn(
                                                "Could not generate executor for '"
                                                        + alias
                                                        + "'. Any errors logged above.");
                        });
        clearAliasQueue();
    }

    private CommandSpec buildInheritedContextExecutor(
            CommandInheritanceContext context, String alias) {
        CommandSpec.Builder builder = this.buildContextExecutor(context, alias);
        Selene.log().info("Children for /" + alias);
        context
                .getInheritedCommands()
                .forEach(
                        inheritedContext ->
                                inheritedContext
                                        .getAliases()
    ...
    private CommandSpec.Builder buildContextExecutor(
            AbstractRegistrationContext context, String alias) {
        CommandSpec.Builder builder = CommandSpec.builder();

        List<AbstractArgumentElement<?>> elements =
                super.parseArgumentElements(
                        context.getCommand().usage(), context.getCommand().permission());
