...
/**
 * A {@link ComponentProviderStrategy} which redirects the component request to the global provider
 * if the request is for a component that is also present in the {@link ComponentRegistry}. If the
 * {@link InjectionCapableApplication#defaultProvider()} is not
 * {@link ComponentRegistryAwareComponentProvider aware of the component registry}, no redirects
 * will take place.
 *
                ...
                boolean isScopedRequest = componentKey.scope()
                    ...
                    .test(key -> {
                        ScopeKey applicationScope =
                            chain.application().defaultProvider().scope().installableScopeType();
