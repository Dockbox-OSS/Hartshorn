...
/**
 * Utilities for working with {@link ComponentContainer}s, specifically for generating IDs and names. If a type
 * is known to a given {@link ComponentRegistry}, the ID and name of the container that contains the type is used.
 *
 * @since 0.4.10
 *
 * @author Guus Lieben
...
public final class ComponentUtilities {
     ...
     * @return the ID
     */
    public static String id(ComponentRegistry registry, Class<?> type) {
        return id(registry, type, false);
    }
     ...
     * @return the ID
     */
    public static String id(ComponentRegistry registry, Class<?> type, boolean ignoreContainerName) {
        return format(registry, type, ignoreContainerName, '-', ComponentContainer::id).toLowerCase(Locale.ROOT);
    }
     ...
     * @return the name
     */
    public static String name(ComponentRegistry registry, Class<?> type, boolean ignoreContainerName) {
        return format(registry, type, ignoreContainerName, ' ', ComponentContainer::name);
    }
     ...
     * @return the name
     */
    public static String format(ComponentRegistry registry, Class<?> type, boolean ignoreContainerName, char delimiter, Function<ComponentContainer<?>, String> attribute) {
        Option<ComponentContainer<?>> container = registry.container(type);
        if (container.present()) {
            return format(container.get(), ignoreContainerName, delimiter, attribute);
        }
        TypeView<?> typeView = registry.injectorContext().environment().introspector().introspect(type);
