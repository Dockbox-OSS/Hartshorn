...
public final class Exceptional<T> {
   ...
   * @param optional The {@link Optional} instance to rewrap
   ...
   *
   * @param <T> The type of the value provided
   ...
   * Exceptional#ofNullable(Object)} is returned. This also means suppliers can return nullable
   * values.
   *
   * @param <T> The type parameter of the potential value
   * @param supplier The {@link Callable} instance, supplying the value
   * @return The {@code Exceptional}
   */
   ...
   *
   * @param <T> The type parameter of the absent value
   ...
   *
   * @param runnable Block to be executed if a value is absent
   * @return The {@code Exceptional}, for chaining
   ...
   *     present and the value matches the given predicate, otherwise an empty {@code Exceptional}
   ...
   *     value of this {@code Exceptional}, if a value is present, otherwise an empty {@code
   *     Optional}
   * @throws NullPointerException If the mapping function is null
   */
  public <U> Exceptional<U> map(Function<? super T, ? extends U> mapper) {
    Objects.requireNonNull(mapper);
    if (!this.isPresent()) return this.errorPresent() ? of(this.throwable) : empty();
    else {
      try {
  ...
  /**
   * Provides a {@code Exceptional} instance which contains both a value in {@link
   * Exceptional#value} and a throwable in {@link Exceptional#throwable}. This requires both the
   * value and the throwable to be non-null. If either is null, a {@link NullPointerException} is
   * thrown.
   *
   * @param <T> The type parameter of the absent value
   * @param value The value to wrap
   ...
   * {@code Exceptional}, and if invoked, {@code flatMap} does not wrap it with an additional {@code
   * Exceptional}.
   *
   * @param <U> The type parameter to the {@code Exceptional} returned
   * @param mapper A mapping function to apply to the value, if present
  ...
  /**
   * If a throwable is present, wrap it in a new {@link CheckedSeleneException} and throw the
   * wrapped exception, otherwise do nothing.
   *
   * @return The {@code Exceptional}, for chaining
   * @throws CheckedSeleneException If {@code throwable} is not null and is rethrown
   */
  public Exceptional<T> rethrow() throws CheckedSeleneException {
    if (null != this.throwable) throw new CheckedSeleneException(this.throwable);
    return this;
  }
   ...
   *
   * @return The {@code Exceptional}, for chaining
   * @throws UncheckedSeleneException If {@code throwable} is not null and is rethrown
   */
   ...
   * throwable is present. This method is similar to {@link Exceptional#orElseGet(Supplier)}, but
