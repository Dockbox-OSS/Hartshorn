...
public final class ImageUtil {
    ...
    public static BufferedImage scaleToFit(BufferedImage image, int width, int height) {
        ...
        double horizontalPadding =
                horizontalRatio > verticalRatio ? calculatePadding(image.getWidth(), width) : 0;
        double verticalPadding =
                verticalRatio > horizontalRatio ? calculatePadding(image.getHeight(), height) : 0;
    ...
    public static BufferedImage scale(
            BufferedImage image, BufferedImage background, ScaleMode scaleMode) {
        BufferedImage scaledImg =
                new BufferedImage(
                        background.getWidth(), background.getHeight(), BufferedImage.TYPE_4BYTE_ABGR);
        ...
        int scaledW =
                imgAspectRatio > mapsAspectRatio
                        ? scaledImg.getWidth()
                        : (int) Math.round(scaledImg.getHeight() * imgAspectRatio);
        int scaledH =
                imgAspectRatio > mapsAspectRatio
                        ? (int) Math.round(scaledImg.getWidth() / imgAspectRatio)
                        : scaledImg.getHeight();
    ...
    private static void placeCentered(BufferedImage image, BufferedImage background) {
        ...
        try {
            ...
            printOnImgGraphics.drawImage(
                    image,
                    null,
                    background.getWidth() / 2 - image.getWidth() / 2,
                    background.getHeight() / 2 - image.getHeight() / 2);
        }
        finally {
    ...
    public static void applyDitherPalette(
            BufferedImage image, DitherMode mode, double colorBleedReduction) {
            ...
            if (!mode.isEnabled()) {
                for (int i = 0; i < image.getHeight(); i++) {
                    for (int j = 0; j < image.getWidth(); j++) {
                        ...
                        int colorIndex =
                                closestMapColorIndex(
                                        nontiledMapPixels[i * w * 4 + j * 4 + 3] & 0xFF,
                                        nontiledMapPixels[i * w * 4 + j * 4 + 2] & 0xFF,
                                        nontiledMapPixels[i * w * 4 + j * 4 + 1] & 0xFF);
                ...
                if (mode.isOrdered()) applyBayer(image, mode, nontiledMapData, nontiledMapPixels);
                    ...
                    applyErrorDiffusion(image, mode, colorBleedReduction, nontiledMapData, nontiledMapPixels);
            }
        }
    }

    private static void applyBayer(
            RenderedImage image, DitherMode mode, byte[][] nontiledMapData, byte[] nontiledMapPixels) {
                    ...
                    int newR =
                            (int)
                                    Math.round(
                                            R
                                                    + (256 / 6.5)
                                                    * (map_value - 0.5)); // This is really bad for indexed palette,
                    int newG =
                            (int)
                                    Math.round(
                                            G
                                                    + (256 / 7.0)
                                                    * (map_value - 0.5)); // and not really optimized for it, but it
                    int newB =
                            (int)
                                    Math.round(
                                            B
                                                    + (256 / 12.5)
                                                    * (map_value - 0.5)); // will make do. Yliluoma1 is too slow for
    ...
    private static void applyErrorDiffusion(
            RenderedImage image,
            DitherMode mode,
            double colorBleedReduction,
            ...
            for (int j = 0; j < image.getWidth(); j++) {
                ...
                if (opaque) {
                    int R =
                            constrainInt(
                                    (nontiledMapPixels[i * w * 4 + j * 4 + 3] & 0xFF)
                                            + (int) Math.round(errorsR[i][j]));
                    int G =
                            constrainInt(
                                    (nontiledMapPixels[i * w * 4 + j * 4 + 2] & 0xFF)
                                            + (int) Math.round(errorsG[i][j]));
                    int B =
                            constrainInt(
                                    (nontiledMapPixels[i * w * 4 + j * 4 + 1] & 0xFF)
                                            + (int) Math.round(errorsB[i][j]));
    ...
    private static int closestMapColorIndex(int R, int G, int B) {
        ...
        for (int i = 4; i < INDEXED_COLORS.length; i++) { // 0-3 are reserved for transparent
            double curDist =
                    colorDistance(
                            R,
                            G,
                            B,
                            (INDEXED_COLORS[i] & 0x00FF0000) >> 16,
                            (INDEXED_COLORS[i] & 0x0000FF00) >> 8,
                            INDEXED_COLORS[i] & 0x000000FF);
