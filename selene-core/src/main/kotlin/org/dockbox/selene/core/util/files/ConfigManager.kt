...
abstract class ConfigManager : ServerReference() {
    ...
    open fun getConfigDir(extension: Class<*>): Path {
        return runWithExtension(extension) { annotation: Extension ->
            getFileType().asPath(
                    Selene.getInstance(FileUtils::class.java).getExtensionConfigdir(),
                    annotation.id
            )
        }
    }
    ...
    open fun getConfigFile(extension: Class<*>): Path {
        return runWithExtension(extension) { annotation: Extension ->
            ...
            Selene.getInstance(FileUtils::class.java).createFileIfNotExists(getFileType().asPath(configPath, annotation.id))
        }
    }
