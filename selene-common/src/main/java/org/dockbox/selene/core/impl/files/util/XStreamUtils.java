...
/**
 * Utility class which configures XStream with some default settings and permissions.
 * For simple usage (custom configuration file with only standard fields and collections):
 * <p>
 * <pre>{@code
 ...
 * }</pre>
 * <p>
 * For complex usage, for example to white list more classes, use a {@link XStreamBuilder} from {@link #create()}.
 *
...
public final class XStreamUtils {
    ...
    /**
     * Reads an XML file with XStream using UTF-8 encoding using the type's ClassLoader
     * and ignoring any IO exception (returning the fall back).
     * You may still can exception if the XML is not as expected. If you do not need backward compatibility,
     * user {@link XStreamBuilder#ignoreUnknownElements()} instead.
     *
     * @param <T>
    ...
    public static final class XStreamBuilder {
        ...
        /**
         * Avoid using this method unless really needed, for example,
         * to configure the stream with properties not supported by the builder
         * or trying to load a legacy xml file.
         *
