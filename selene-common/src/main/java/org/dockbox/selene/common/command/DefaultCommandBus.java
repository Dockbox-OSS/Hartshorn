...
public abstract class DefaultCommandBus implements CommandBus {
   ...
   */
  private static final Pattern GENERIC_ARGUMENT =
      Pattern.compile("((?:<.+?>)|(?:\\[.+?\\])|(?:-(?:(?:-\\w+)|\\w)(?: [^ -]+)?))");
   ...
   * </ul>
   */
  private static final Pattern ELEMENT_VALUE =
      Pattern.compile("(\\w+)(?:\\{(\\w+)(?::([\\w\\.]+))?\\})?");

  private static final Map<String, ConfirmableQueueItem> confirmQueue =
      SeleneUtils.emptyConcurrentMap();

  private static final Map<String, AbstractRegistrationContext> registrations =
      SeleneUtils.emptyConcurrentMap();
  private static final Map<String, List<CommandInheritanceContext>> queuedAliases =
      SeleneUtils.emptyConcurrentMap();
  ...
  private static List<AbstractRegistrationContext> createContexts(Class<?> parent) {
    ...
    */
    ...
    @NotNull
    @Unmodifiable
    Collection<Method> nonInheritedMethods =
        Reflect.getAnnotedMethods(
            parent, Command.class, c -> !c.inherit() || !isParentRegistration);
    nonInheritedMethods.forEach(
        method -> {
  ...
  private static boolean prepareInheritanceContext(
      AbstractRegistrationContext context, String alias) {
    /*
          ...
          .forEach(
              extendingContext ->
                  DefaultCommandBus.addExtendingContextToRegistration(
                      extendingContext, (CommandInheritanceContext) context));
    }
    return false;
  }
  ...
  private static CommandInheritanceContext extractCommandInheritanceContext(Class<?> parent) {
    ...
    */
    @NotNull
    @Unmodifiable
    Collection<Method> inheritedMethods =
        Reflect.getAnnotedMethods(parent, Command.class, Command::inherit);
    inheritedMethods.forEach(
        method ->
            context.addInheritedCommand(
                DefaultCommandBus.extractInheritedContext(method, context)));

    return context;
  }

  private @Nullable static MethodCommandContext extractNonInheritedContext(Method method) {
  ...
  private static void addExtendingAliasToRegistration(
      String alias, CommandInheritanceContext extendingContext) {
  ...
  private static void addExtendingContextToRegistration(
      CommandInheritanceContext extendingContext, CommandInheritanceContext context) {
    extendingContext.getInheritedCommands().forEach(context::addInheritedCommand);
  }

  private @Nullable static MethodCommandContext extractInheritedContext(
      Method method, CommandInheritanceContext parentContext) {
    Command command = method.getAnnotation(Command.class);
    if (!command.inherit()) return null;
    for (String alias : command.aliases()) {
      if (parentContext.getInheritedCommands().stream()
        ...
        */
        Selene.log()
            .warn(
                "Context for '"
                    + parentContext.getPrimaryAlias()
                    + "' has duplicate inherited context for '"
                    + alias
                    + "'.");
      }
    }
  ...
  protected AbstractArgumentValue<?> generateArgumentValue(
      String argumentDefinition, String defaultPermission) {
    ...
    if (!elementValue.matches() || 0 == elementValue.groupCount())
      Selene.handle(
          "Unknown argument specification "
              + argumentDefinition
              + ", use Type or Name{Type} or Name{Type:Permission}");
    ...
    */
    if (1 <= elementValue.groupCount()) {
      ...
      throw new IllegalArgumentException(
          "Missing key argument in specification '" + argumentDefinition + "'");
  ...
  protected List<AbstractArgumentElement<?>> parseArgumentElements(
      CharSequence argString, String defaultPermission) {
    ...
    while (genericArgumentMatcher.find()) {
      ...
      if (argumentMatcher.matches()) {
        ...
        flagCollection =
            this.getAbstractFlagCollection(flagCollection, flagMatcher, defaultPermission);
      }
    }
  ...
  private AbstractFlagCollection<?> getAbstractFlagCollection(
      AbstractFlagCollection<?> flagCollection, Matcher flagMatcher, String defaultPermission) {
  ...
  private void extractArguments(
      Collection<AbstractArgumentElement<?>> elements,
      Matcher argumentMatcher,
      String defaultPermission) {
    boolean optional = '[' == argumentMatcher.group(1).charAt(0);
    String elementValue = argumentMatcher.group(2);

    List<AbstractArgumentElement<?>> result =
        this.parseArgumentElements(elementValue, defaultPermission);
    if (result.isEmpty()) {
      AbstractArgumentValue<?> argumentValue =
          this.generateArgumentValue(argumentMatcher.group(2), defaultPermission);
  ...
  private void parseFlag(
      AbstractFlagCollection<?> flags, String name, String value, String defaultPermission) {
    if (null == value) {
      int at;
      ...
      if (0 <= name.indexOf(':')) {
        Selene.handle(
            "Flag values do not support permissions at flag `"
                + name
                + "`. Permit the value instead");
      }
  ...
  protected static void callCommandContext(
    ...
    */
    if (registrationContext.getCommand().confirm()
        && sender instanceof AbstractIdentifiable
      ...
      confirmText.onHover(
          HoverAction.showText(IntegratedResource.CONFIRM_COMMAND_MESSAGE_HOVER.asText()));
      confirmText.onClick(ClickAction.runCommand("/selene confirm " + registrationId));
      sender.sendWithPrefix(confirmText);

    } else {
      Exceptional<IntegratedResource> response =
          DefaultCommandBus.callCommandWithEvents(sender, ctx, command, registrationContext);

      if (response.errorPresent())
        sender.sendWithPrefix(
            IntegratedResource.UNKNOWN_ERROR.format(response.getError().getMessage()));
    }
  }
  ...
  protected abstract AbstractArgumentElement<?> wrapElements(
      List<AbstractArgumentElement<?>> elements);

  protected abstract AbstractArgumentValue<?> generateArgumentValue(
      String type, String permission, String key);
