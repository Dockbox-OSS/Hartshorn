...
public abstract class DefaultDiscordUtils implements DiscordUtils {
  ...
  private static final Map<String, Triad<DiscordCommand, Method, Object>> commandMethods =
  ...
  public Exceptional<Category> getLoggingCategory() {
    if (this.getJDA().isPresent())
      return Exceptional.ofNullable(
          this.getJDA()
              .get()
              .getCategoryById(Selene.getServer().getGlobalConfig().getDiscordLoggingCategoryId()));
    ...
    Arrays.stream(obj.getClass().getDeclaredMethods())
        ...
        .forEach(
            method -> {
              ...
              Triad<DiscordCommand, Method, Object> information =
                  new Triad<>(annotation, method, instance);

              if (commandMethods.containsKey(command))
                Selene.log()
                    .warn("More than one listener registered for Discord command: " + command);
  ...
  public void post(@NotNull String command, @NotNull DiscordCommandContext context) {
    if (commandMethods.containsKey(command)) {
      ...
      boolean correctChannel =
          WILDCARD.equals(annotation.channelId())
      ...
      if (!userPermitted) {
        ...
        Exceptional<Role> or =
            this.getGuild().map(guild -> guild.getRoleById(annotation.minimumRankId()));
      ...
      try {
        ...
        Selene.handle(
            "Failed to invoke previously checked method ["
                + method.getName()
                + "] in ["
  ...
  private static boolean isValidChannel(
      @NotNull DiscordCommandContext context, ListeningLevel level) {
    boolean listensForBoth = ListeningLevel.BOTH == level;
    if (listensForBoth) return true;

    boolean isPrivateAndValid =
        ListeningLevel.PRIVATE_ONLY == level
            && ChannelType.PRIVATE == context.getChannel().getType();
    boolean isTextAndValid =
        ListeningLevel.CHANNEL_ONLY == level && ChannelType.TEXT == context.getChannel().getType();

    return isPrivateAndValid || isTextAndValid;
  }

  private static void sendToTextChannel(
      @NotNull CharSequence text, @NotNull MessageChannel channel) {
        ...
        .ifPresent(
            jda -> {
              try {
                ...
                channel
                    .sendMessage((Message) pages.get(0).getContent())
  ...
  public void sendToUser(DiscordPagination pagination, User user) {
    user.openPrivateChannel()
        .queue(privateChannel -> sendToTextChannel(pagination, privateChannel));
  }
