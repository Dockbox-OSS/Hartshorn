...
public class PaintingsDiscordListener {
    ...
    public void onDiscordChat(DiscordEvent.MessageEvent event)
        ...
        HashMap<Integer, PaintingSubmission> submissionHashMap = DarwinServer.getModule(PaintingsModule.class)
                .get().submissions;
        ...
        if ((!playername.equals("DR")) && event.getChannel().getId().equals(CHANNEL_ID)) {
            ...
            if (message.startsWith("!approve")) {
                ...
                if (submissionHashMap.containsKey(id) && submissionHashMap.get(id).getStatus()
                        .equals(Translations.PAINTING_STATUS_SUBMITTED
                                .s())) {
                    PaintingSubmission submission = submissionHashMap.get(id);
                    Sponge.getCommandManager().process(Sponge.getServer().getConsole(), "mail send " + PlayerUtils
                            .getSafely(PlayerUtils
                                    .getNameFromUUID(submission
                                            .getPlayerUUID())) + " your submission of " + submission
                            .getCommand() + " was approved, use /paintingslist to obtain it.");
                    Sponge.getCommandManager().process(Sponge.getServer().getConsole(), "uploadpainting " + PlayerUtils
                            .getSafely(PlayerUtils
                                    .getNameFromUUID(submission
                                            .getPlayerUUID())) + " " + submission
                            .getCommand());
                    ...
                    DiscordHandler.sendMessageToChannel(CHANNEL_ID, Translations.PAINTING_CANNOT_UPDATE_STATUS
                            .f("approve", id));
                }
                ...
                DiscordHandler.sendMessageToChannel(CHANNEL_ID, Translations.PAINTING_SUBMISSION_LIST
                        .f(submissionHashMap.keySet()));
                ...
                try (ResultSet results = stmt.executeQuery()) {
                    ...
                    while (results.next()) {
                        exists = true;
                        String playerName = PlayerUtils.getSafely(PlayerUtils
                                .getNameFromUUID(UUID.fromString(results.getString("PlayerUUID"))));
                        String status = results.getString("Status");
                        Color color = status.equals(Translations.PAINTING_STATUS_SUBMITTED.s()) ? Color.YELLOW :
                                status.equals(Translations.PAINTING_STATUS_REJECTED
                                        .s()) ? Color.RED :
                                        status.equals(Translations.PAINTING_STATUS_APPROVED
                                                .s()) ? Color.GREEN : null;
                ...
                DiscordHandler
                        .sendMessageToChannel(CHANNEL_ID, "Use !approve <ID> to approve a submission and !reject <ID> to reject a submission.");
                DiscordHandler
                        .sendMessageToChannel(CHANNEL_ID, "Use !list to see all submissions awaiting approval/rejection.");
