...
/**
 * The default method interceptor used by the
 * {@link org.dockbox.hartshorn.proxy.JDKInterfaceProxyFactory}. This interceptor is responsible for
 * invoking the {@link ProxyMethodInterceptHandler} and validating the results of the invocation.
 *
 * <p>This interceptor ensures a state is prepared for the {@link ProxyMethodInterceptHandler} to
 * use. This state
 * includes the {@link ProxyCallbackContext} and the {@link CustomInvocation} which is used to
 * invoke the method on the target instance. The actual invocation is delegated to the
 * {@link ProxyMethodInterceptHandler}, which will typically handle the invocation using configured
 * advisors in its {@link ProxyMethodInvoker}.
 *
...
public class ProxyAdvisorMethodInterceptor<T> implements ProxyMethodInterceptor<T> {
    ...
    public ProxyAdvisorMethodInterceptor(
    ...
    /**
     * Intercepts the method invocation and delegates the invocation to the
     * {@link ProxyMethodInterceptHandler}. This may involve invoking an advised interceptor, or
     * simply invoking the method on the target instance, depending on the configuration of the
     * {@link ProxyManager#advisor() proxy manager's advisor}.
     *
     ...
     */
    protected Object interceptAndNotify(
        T self, MethodInvokable source, Invokable proxy, T callbackTarget,
        MethodView<T, ?> methodView, CustomInvocation<?> defaultInvocation,
        Object[] arguments
    ) throws Throwable {

        ProxyCallbackContext<T> callbackContext = new ProxyCallbackContext<>(callbackTarget,
            TypeUtils.unchecked(self, Object.class),
            methodView,
            arguments);
        return this.manager().advisor().safeWrapIntercept(callbackContext, () -> {
            ...
            if (interceptor.present()) {
                return this.interceptHandler.handleInterceptedMethod(source,
                    callbackTarget,
                    defaultInvocation,
                    arguments,
                    interceptor.get());
                ...
                return this.interceptHandler.handleNonInterceptedMethod(self,
                    source,
                    proxy,
                    callbackTarget,
                    arguments);
    ...
    /**
     * Creates a default invocation for the method. This is used when no interceptor is present, or
     * when the interceptor does not handle the method. The default invocation can either invoke the
     * method on the delegate instance (if present), or return a default value based on the method's
     * return type.
     *
     * @param source the method being invoked, represented as a {@link MethodInvokable}
     * @param proxy the proxy method being invoked, which may be null if the method is not proxied
     ...
     */
    protected CustomInvocation<?> createDefaultInvocation(
        Invokable source,
        Invokable proxy,
        T callbackTarget
    ) {
        return interceptorArgs -> {
            if (this.manager().delegate().present()) {
                return this.methodInvoker.invokeDelegate(this.manager().delegate().get(),
                    source,
                    interceptorArgs);
            }
            if (proxy == null) {
    ...
    /**
     * Resolves the arguments for the method invocation, using the
     * {@link #parameterLoader configured parameter loader}. This typically only involves unproxying
     * the arguments, but may also involve additional processing if the method has parameters that
     * require special handling (e.g., annotations, default values).
     *
     * @param method the method being invoked, represented as a {@link MethodInvokable}
     * @param instance the instance on which the method is invoked, which may be a proxy or a
     * concrete instance
     * @param args the arguments passed to the method invocation, which may include proxies or other
     * objects
     *
    ...
    protected Object[] resolveArgs(MethodInvokable method, Object instance, Object[] args) {
        ...
        ProxyParameterLoaderContext context =
            new ProxyParameterLoaderContext(methodView, instance, this.proxyOrchestrator);
        return this.parameterLoader().loadArguments(context, args).toArray();
    }

    /**
     * Returns the {@link ParameterLoader} used to load the parameters for the method invocation.
     * This loader is responsible for unproxying the arguments and resolving any special handling
     * required for the method's parameters.
     *
