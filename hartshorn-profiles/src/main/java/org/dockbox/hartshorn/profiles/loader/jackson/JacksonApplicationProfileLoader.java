...
public abstract class JacksonApplicationProfileLoader implements ProfilePropertiesLoader {
    ...
    @Override
    public Set<ValueProfileProperty> loadProperties(URI uri) throws ApplicationException {
        try {
            ObjectMapper mapper = this.createObjectMapper();
            InputStream inputStream = uri.toURL().openStream();
            JsonNode node = mapper.readTree(inputStream);
            return this.loadProperties(node);
        }
        catch (IOException e) {
    ...
    private Set<ValueProfileProperty> loadProperties(JsonNode node) {
