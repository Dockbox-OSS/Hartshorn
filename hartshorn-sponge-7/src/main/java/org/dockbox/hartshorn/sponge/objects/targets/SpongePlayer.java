...
import org.dockbox.hartshorn.server.minecraft.events.chat.SendMessageEvent;
...
import org.dockbox.hartshorn.sponge.objects.composite.SpongeComposite;
import org.dockbox.hartshorn.sponge.objects.inventory.SpongePlayerInventory;
...
public class SpongePlayer extends Player implements SpongeComposite, Wrapper<org.spongepowered.api.entity.living.player.Player> {
    ...
    private final ApplicationContext context = Hartshorn.context();
    ...
    @Override
    public void setPermission(String permission, org.dockbox.hartshorn.api.domain.tuple.Tristate state) {
        this.setPermission(permission, SubjectData.GLOBAL_CONTEXT, state);
    }

    @Override
    public void setPermission(Permission permission, org.dockbox.hartshorn.api.domain.tuple.Tristate state) {
        if (permission.getContext().absent()) {
    ...
    private void setPermission(String permission, Set<Context> contexts, org.dockbox.hartshorn.api.domain.tuple.Tristate state) {
            ...
            Sponge.getServiceManager().provide(PacketGate.class)
                    .ifPresent(packetGate -> {
                        ...
                        connection.present(packetConnection -> {
                            ...
                            Hartshorn.log().warn("Could not create packet connection for player '" + this.getName() + "'");
                        });
                    });
        }
    }
