...
public class SpongePlayerListener {
    ...
    public void on(
        ...
        SpongePlayerListener.postIfCommandSource(source, convertedSource -> {
            Cancellable event = new PlayerBannedEvent(
                    ...
                    HartshornUtils.toLocalDateTime(profile.getExpirationDate()),
                    HartshornUtils.toLocalDateTime(profile.getCreationDate())
            ).post();
            if (event.isCancelled()) SpongePlayerListener.logUnsupportedCancel(event);
        });
    }

    private static void postIfCommandSource(Object source, Consumer<org.dockbox.hartshorn.commands.source.CommandSource> consumer) {
    ...
    private static void logUnsupportedCancel(Cancellable event) {
        Hartshorn.log().warn("Attempted to cancel event of type '" + event.getClass().getSimpleName() + "', but this is not supported on this platform!");
    }
    ...
    private static void logUnsupportedModification(Event event) {
        Hartshorn.log().warn("Attempted to modify value 'reason' event of type '" + event.getClass().getSimpleName() + "', but this is not supported on this platform!");
    }
    ...
    public void on(
        ...
        org.dockbox.hartshorn.server.minecraft.entities.Entity targetEntity = SpongeConversionUtil.fromSponge(entity);
