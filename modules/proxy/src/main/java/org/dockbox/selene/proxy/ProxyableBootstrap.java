...
public final class ProxyableBootstrap {
    ...
    static void boostrapDelegates() {
        ...
        Reflect.getAnnotatedTypes(SeleneInformation.PACKAGE_PREFIX, Proxy.class)
                .forEach(
                        proxy -> {
                            Selene.log().info("Processing [" + proxy.getCanonicalName() + "]");
                            if (Modifier.isAbstract(proxy.getModifiers())) {
                                Selene.log()
                                        .warn("Proxy source cannot be abstract [" + proxy.getCanonicalName() + "]");
                                return;
                            }
                            ...
                            if (delegationInfo.value().isAnnotationPresent(Proxy.class)) {
                                Selene.log()
                                        .warn(
                                                "Proxy target cannot be another delegate ["
    ...
    private static void delegateMethods(Class<?> proxyClass) {
        ...
        Collection<Method> targets =
                Reflect.getAnnotedMethods(proxyClass, Target.class, i -> true, false);
    ...
    private static <T, C> void delegateMethod(
            Class<T> proxyClass, Class<C> proxyTargetClass, Method source) {
        ...
        // target method.
        Class<?>[] arguments =
                SeleneUtils.asList(source.getParameters()).stream()
        ...
        try {
            ...
            Selene.log()
                    .warn(
                            "Proxy target does not have declared method '"
                                    + methodName
                                    + "("
